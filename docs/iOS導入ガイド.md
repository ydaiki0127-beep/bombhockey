...
## 4.5 Sideloadly 完全ガイド（ダウンロード→信頼設定まで）

### ① 必要なもの（事前準備）

- **Windows PC**（7以降）
- **iPhone**（このゲームはiOS 18以降が必要）
- **USBケーブル**（純正またはMFI認証品。充電専用ケーブルはNG）
- **Apple ID**（無料でOK。Gmailでも作成可能）
- **Appleのデバイスドライバ**（Windowsで必須）:
  - 推奨: Microsoft Store の「**Apple Devices**」アプリ
  - または: 旧「**iTunes**」（Apple公式サイトから）

### ② Sideloadly のダウンロードとインストール

1. ブラウザで **https://sideloadly.io** を開く
2. 「**Windows**」のダウンロードボタンをクリック
3. ダウンロードした **ZIPファイルを右クリック → すべて展開**（解凍）
4. 解凍フォルダ内の **Sideloadly.exe** をダブルクリックで起動
   - スマートスクリーンが警告したら「**詳細情報**」→「**実行**」

### ③ iPhone と PC を接続

1. USBケーブルでiPhoneをPCに接続
2. iPhone画面に「このコンピュータを信頼しますか？」→「**信頼**」→ パスコード入力
3. （Apple Devices/iTunesが入っていれば）PCでiPhoneが認識されていることを確認

### ④ Sideloadly で署名・インストール

1. Sideloadly を起動
2. 上部の **IPA** 欄に `bombhockey-unsigned.ipa` をドラッグ＆ドロップ
3. **Apple ID** 欄にメールアドレス、下の欄にパスワードを入力
4. **Start** ボタンをクリック
5. 初回のみApple IDの確認ダイアログ → 画面の指示に従う（2段階認証コードの入力など）
6. 「**Installation successful**」と出るまで待つ（数分）

### ⑤ iPhone での信頼設定（重要）

1. iPhoneの「**設定**」アプリを開く
2. 「**一般**」→「**VPNとデバイス管理**」
3. 「**デベロッパApp**」欄に自分のApple IDが出ているのでタップ
4. 「**信頼**」をタップ → 確認ダイアログで再度「信頼」
5. ホーム画面に戻ると「ボムホッケー」アイコンが出現 → タップで起動！

### ⑥ 7日ごとの再署名について

- 無料Apple IDの署名は **7日で失効**。失効するとアイコンがグレーアウトして起動不可
- Sideloadlyの**自動リフレッシュ機能**（Auto-Refresh）を有効にすると、
  PC起動中に自動で再署名してくれる（Settings でオンにできる）
- 手動で行う場合: Sideloadlyでもう一度 Start を押すだけ
- 制限: 同時にサイドロードできるアプリは **3個まで**
...
| アプリが7日後に起動しない | 再署名する（SideloadlyでStart、または自動リフレッシュ） |
